  ...
  return (
    <main className="pt-24 pb-32 px-4 max-w-2xl mx-auto min-h-screen">
      <section className="mb-10 text-center">
        <h2 className="font-display-lg-mobile md:font-display-lg text-secondary drop-shadow-[0_0_30px_rgba(254,0,254,0.6)] text-glow-magenta uppercase italic">Mi Zona Segura</h2>
        <div className="h-1 w-32 acid-gradient mx-auto my-6"></div>
        <p className="text-secondary font-label-caps drop-shadow-[0_0_10px_rgba(254,0,254,0.4)]">Tu espacio personal de paz y reflexión</p>
      </section>
        ...
        <div className="grid grid-cols-2 gap-3">
          {affirmations.map((a) => (
            <div key={a.text} className="glass-card p-5 rounded-xl border-primary/20 text-center group hover:border-primary/50 transition-all">
              ...
              <p className="font-body-md font-bold mt-3 leading-tight">{a.text}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="mb-8">
        ...
        <div className="glass-card p-6 rounded-xl border-secondary/30">
          <p className="font-body-md text-on-surface-variant mb-4">Escribe lo que sientes. Al quemarlo, lo liberas.</p>
          ...
          {playlists.map((p) => (
            <div key={p.name} className="glass-card p-5 rounded-xl border-primary/20">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <p className="font-label-caps text-primary">{p.name}</p>
                  <p className="font-label-md text-on-surface-variant">{p.desc}</p>
                </div>
              </div>
